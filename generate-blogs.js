...
function init4DLogoEngine() {
  ...
  containers.forEach(container => {
    ...
    // Use alpha: true and no EffectComposer to perfectly match the website CSS background!
    ...
    shape.moveTo(-1, -1);
    shape.lineTo(-1, 1);
    shape.lineTo(-0.2, 0.2); // inner left dip
    shape.lineTo(0.2, 0.2);  // inner right dip
    shape.lineTo(1, 1);
    shape.lineTo(1, -1);
    shape.lineTo(0.5, -1);
    ...
    // 2. High-End Glass Material (MeshPhysicalMaterial)
    const glassMaterial = new THREE.MeshPhysicalMaterial({
      color: 0xffffff,
      metalness: 0.2,
      roughness: 0.05,
      transmission: 1.0, // Flawless glass
      ior: 1.5,
      thickness: 1.5,
      clearcoat: 1.0,
    ...
    function animate() {
      ...
      // Rotate the orbiting data orbs around the glass M
      orbGroup.rotation.y += delta * 1.5;
      orbGroup.rotation.z += delta * 0.5;
