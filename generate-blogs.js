...
function generateHTML(post) {
  ...
  const LOGO_SVG = `<div class="webgl-logo-container logo-icon" style="width: 44px; height: 44px; display: inline-flex; justify-content: center; align-items: center;"></div>`;
...
<!-- Three.js CDN for WebGL 4D Interactive Engine -->
...
<script>
function init4DLogoEngine() {
  if (typeof THREE === 'undefined') return;

  const containers = document.querySelectorAll('#webgl-reveal-container, .webgl-logo-container');

  containers.forEach(container => {
    if (container.dataset.webglInited) return;
    container.dataset.webglInited = 'true';

    const isReveal = container.id === 'webgl-reveal-container';
    const width = isReveal ? 400 : 44;
    const height = isReveal ? 400 : 44;

    const scene = new THREE.Scene();
    const camera = new THREE.PerspectiveCamera(45, width / height, 0.1, 100);
    camera.position.z = isReveal ? 5.5 : 4.5;
    ...
    // --- PRO ENHANCEMENT: Dynamic Environment Map for Realistic Glass Reflections ---
    ...
    grad.addColorStop(0.3, '#22d3ee'); // Cyan reflection
    grad.addColorStop(0.7, '#c084fc'); // Purple reflection
    grad.addColorStop(1, '#020617');
    ctx.fillStyle = grad;
    ctx.fillRect(0, 0, 512, 512);
    ctx.fillStyle = '#ffffff'; // Bright light sources for sharp specular hits
    ctx.beginPath(); ctx.arc(128, 128, 40, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.arc(384, 384, 60, 0, Math.PI*2); ctx.fill();
    ...
    const extrudeSettings = {
      depth: 0.4,
      bevelEnabled: true,
      bevelSegments: 8, // Pro: Smoother bevels
      bevelSteps: 4,
      bevelThickness: 0.08,
      bevelSize: 0.08
    };
    ...
    // 2. Pro Glass Material (MeshPhysicalMaterial with Attenuation)
    const glassMaterial = new THREE.MeshPhysicalMaterial({
      color: 0xffffff,
      metalness: 0.1,
      roughness: 0.02, // Flawless surface
      transmission: 1.0, // Fully glass
      ior: 1.65, // High index of refraction for heavy distortion
      thickness: 1.5,
      attenuationColor: new THREE.Color(0xc084fc), // Absorbs light, giving thick parts a deep purple tint
      attenuationDistance: 1.5,
      clearcoat: 1.0,
      clearcoatRoughness: 0.05,
      transparent: true,
      envMapIntensity: 2.0 // Boosts reflections
    });
    
    const mMesh = new THREE.Mesh(geometry, glassMaterial);
    
    // --- PRO ENHANCEMENT: Inner Metallic Core ---
    // A dark metallic core sitting inside the glass makes it look incredibly sophisticated
    ...
    coreMesh.scale.set(0.85, 0.85, 0.6); // Scale down to fit inside the glass casing
    mMesh.add(coreMesh);
    
    if(!isReveal){
        mMesh.scale.set(1.5, 1.5, 1.5);
    }
    mainGroup.add(mMesh);

    // 3. Orbiting AI Data Orbs with Faux Glow
    const orbGroup = new THREE.Group();
    const orbGeo = new THREE.SphereGeometry(isReveal ? 0.15 : 0.3, 32, 32);
    const glowGeo = new THREE.SphereGeometry(isReveal ? 0.3 : 0.6, 32, 32);
    
    // Cyan Orb
    const orbMat1 = new THREE.MeshBasicMaterial({ color: 0x22d3ee }); 
    const orb1 = new THREE.Mesh(orbGeo, orbMat1);
    orb1.position.set(1.6, 0.4, 0);
    const glowMat1 = new THREE.MeshBasicMaterial({ color: 0x22d3ee, transparent: true, opacity: 0.3, blending: THREE.AdditiveBlending });
    orb1.add(new THREE.Mesh(glowGeo, glowMat1)); // Additive glow
    orbGroup.add(orb1);
    
    // Purple Orb
    const orbMat2 = new THREE.MeshBasicMaterial({ color: 0xc084fc }); 
    const orb2 = new THREE.Mesh(orbGeo, orbMat2);
    orb2.position.set(-1.6, -0.4, 0);
    const glowMat2 = new THREE.MeshBasicMaterial({ color: 0xc084fc, transparent: true, opacity: 0.3, blending: THREE.AdditiveBlending });
    orb2.add(new THREE.Mesh(glowGeo, glowMat2)); // Additive glow
    orbGroup.add(orb2);
    ...
    const pointLight1 = new THREE.PointLight(0x22d3ee, 3, 10);
    pointLight1.position.set(-3, 3, 3);
    scene.add(pointLight1);

    const pointLight2 = new THREE.PointLight(0xc084fc, 3, 10);
    ...
    function animate() {
      ...
      // Dynamic cinematic orbiting
      orbGroup.rotation.y += delta * 1.5;
      orbGroup.rotation.z += delta * 0.8;
      orbGroup.rotation.x += delta * 0.4;

      // Pulse the orbs slightly
      const scale = 1.0 + Math.sin(time * 4) * 0.15;
      orb1.scale.set(scale, scale, scale);
      orb2.scale.set(scale, scale, scale);

      targetX += (mouseX - targetX) * 0.05;
      targetY += (mouseY - targetY) * 0.05;
      
      mainGroup.rotation.y += delta * 0.3;
...
document.addEventListener('DOMContentLoaded', init4DLogoEngine);
</script>
\n</head>
<body>
