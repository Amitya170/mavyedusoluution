...
function init4DLogoEngine() {
  ...
  containers.forEach(container => {
    ...
    camera.position.z = isReveal ? 6.0 : 4.0;
    ...
    // 1. Background Globe Network (Particles)
    const particleCount = isReveal ? 1000 : 300;
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      bgPositions[i * 3] = radius * Math.cos(theta) * Math.sin(phi);
    ...
    const bgMaterial = new THREE.PointsMaterial({
      color: 0x3b82f6,
      size: isReveal ? 0.05 : 0.02,
      transparent: true,
      opacity: 0.4,
      blending: THREE.AdditiveBlending
    });
    const bgPoints = new THREE.Points(bgGeometry, bgMaterial);
    mainGroup.add(bgPoints);

    // 2. The Neural "M" points
    const mRadius = 2.1;
    // Points forming an 'M'
    const mNodes = [
      new THREE.Vector3(-1.0, -1.0, 1.0), // 0: Bottom left
      new THREE.Vector3(-1.0, 0.0, 1.0),  // 1: Mid left
      new THREE.Vector3(-1.0, 1.0, 1.0),  // 2: Top left
      new THREE.Vector3(-0.5, 0.5, 1.2),  // 3: Inner left diag
      new THREE.Vector3(0.0, 0.0, 1.4),   // 4: Center bottom
      new THREE.Vector3(0.5, 0.5, 1.2),   // 5: Inner right diag
      new THREE.Vector3(1.0, 1.0, 1.0),   // 6: Top right
      new THREE.Vector3(1.0, 0.0, 1.0),   // 7: Mid right
      new THREE.Vector3(1.0, -1.0, 1.0)   // 8: Bottom right
    ];
    
    // Normalize and project onto sphere
    mNodes.forEach(p => {
      p.normalize().multiplyScalar(mRadius);
    });

    const mGroup = new THREE.Group();
    
    // Create glowing nodes for the 'M'
    const mNodeGeo = new THREE.SphereGeometry(isReveal ? 0.1 : 0.05, 16, 16);
    const mNodeMat = new THREE.MeshBasicMaterial({
      color: 0x22d3ee, // Cyan
      transparent: true,
      opacity: 0.9,
    });
    ...
    mNodes.forEach(p => {
      const mesh = new THREE.Mesh(mNodeGeo, mNodeMat);
      mesh.position.copy(p);
      mGroup.add(mesh);
    });

    // Create connections (Neural Links) for the 'M'
    const edges = [
      [0,1], [1,2], // Left leg
      [2,3], [3,4], // Left diag
      [4,5], [5,6], // Right diag
    ...
    const mLinePos = new Float32Array(edges.length * 6);
    let lIdx = 0;
    edges.forEach(edge => {
      const p1 = mNodes[edge[0]];
      const p2 = mNodes[edge[1]];
      mLinePos[lIdx++] = p1.x; mLinePos[lIdx++] = p1.y; mLinePos[lIdx++] = p1.z;
      mLinePos[lIdx++] = p2.x; mLinePos[lIdx++] = p2.y; mLinePos[lIdx++] = p2.z;
    });
    ...
    const mLines = new THREE.LineSegments(mLineGeo, mLineMat);
    mGroup.add(mLines);

    mainGroup.add(mGroup);

    // Subtle background lines linking random nodes on the globe
    const bgEdgesCount = isReveal ? 300 : 50;
    const bgLinePos = new Float32Array(bgEdgesCount * 6);
    for(let i=0; i<bgEdgesCount; i++){
       let idx1 = Math.floor(Math.random() * particleCount);
       let idx2 = Math.floor(Math.random() * particleCount);
       bgLinePos[i*6+0] = bgPositions[idx1*3];
       bgLinePos[i*6+1] = bgPositions[idx1*3+1];
       bgLinePos[i*6+2] = bgPositions[idx1*3+2];
       bgLinePos[i*6+3] = bgPositions[idx2*3];
       bgLinePos[i*6+4] = bgPositions[idx2*3+1];
       bgLinePos[i*6+5] = bgPositions[idx2*3+2];
    }
    ...
    mainGroup.add(new THREE.LineSegments(bgLineGeo, bgLineMat));
    ...
    function animate() {
      ...
      // Breathing animation for the M nodes
      mGroup.children.forEach((child, index) => {
        if (child.isMesh) {
          const scale = 1.0 + Math.sin(time * 2 + index) * 0.2;
      ...
      mainGroup.rotation.y += delta * 0.2;
      mainGroup.rotation.x += delta * 0.1;
      
      if (isReveal) {
        mainGroup.rotation.y += targetX * 0.1;
